layui.define(["form", "jquery"], function (exports) {
    ...
    var dongxiHttp =
        {
            get(cmd, param) {
                return request('get', cmd, param);
            },
            post(cmd, param) {
                return request('post', cmd, param);
            }
        }
        ...
        return new Promise((resove, reject) => {
            ...
            function parseParams(obj) {
                try {
                    ...
                    for (var item in sortKey) {
                        let key = sortKey[item],
                            value = obj[key];
                        if (typeof (value) != 'object')
                            urlArray.push(`${key}=${value}`);
                    }
                    var urlParamsStr = urlArray.join('&');
                    return urlParamsStr;
                } catch (err) {
                    return '';
                }
            ...
            // k1 = v1 & k2=v2 & secretKey
            var secretKey = 'e44163af-9872-4567-a512-a1a67b517e70';
            var signStr = ""
            if (params) {
                signStr = `${params}&${secretKey}`
            } else {
                signStr = `${secretKey}`
            }
